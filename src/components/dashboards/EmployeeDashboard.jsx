...
function EmployeeDashboard() {
    ...
    React.useEffect(() => {
        ...
        const today = new Date().toISOString().split('T')[0]
        const employeeId = user.employee_table_id || user.id || 1
        const todayAttendance = attendance.find(a =>
            String(a.employee_id) === String(employeeId) &&
            a.date === today &&
            !a.clock_out // Only find OPEN sessions
        )

        console.log('Attendance Check:', { today, employeeId, found: todayAttendance });
    ...
    const handleAutoLogout = async () => {
        try {
            const employeeId = user.employee_table_id || user.id || 1
            const res = await clockOut(employeeId, null, null)
            if (res.success) {
                setStatus('checked-out')
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-gray-600 font-bold uppercase text-[10px] tracking-widest mt-1">Welcome back, {user.name}</p>
                </div>
                <div className="flex gap-4 items-center">
                    {/* Attendance Clock In/Out */}
                    <div className="flex items-center gap-3 bg-white p-2 rounded-xl border border-gray-200 shadow-md">
                        <div className="text-right mr-2 hidden md:block">
                            <p className="text-[9px] font-black uppercase tracking-widest text-gray-600">Attendance Log</p>
                            {status === 'checked-in' && (
                                <div className="flex items-center gap-1 mt-1 bg-green-50 px-2 py-0.5 rounded-full border border-green-100">
                                    <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                                    <p className="text-[9px] font-black text-green-700 uppercase">Active Now</p>
                                </div>
                            )}
                            {lastActionTime && <p className="text-[10px] font-black text-gray-900">Last: {lastActionTime}</p>}
                        </div>
                        <button
                            onClick={handleAttendanceClick}
                            disabled={isLoading}
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-[10px] font-black uppercase transition-all shadow-lg active:scale-95 ${status === 'checked-in' ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}
                        >
                                    ...
                                    {!capturedImage ? (
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            ...
                                            <button
                                                ...
                                                disabled={isLoading}
                                                className={`py-4 text-white rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-2 ${isLoading ? 'bg-gray-400' : 'bg-emerald-600 hover:bg-emerald-700'}`}
                                            >
                    ...
                    <div className="bg-white px-4 py-2 rounded-xl border border-gray-200 shadow-sm">
                        <p className="text-[10px] font-black uppercase text-gray-600">Current Role</p>
                        <p className="font-black text-gray-900 flex items-center gap-2 uppercase text-sm"><Zap size={14} className="text-vayu-yellow" /> {user.role}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-xs font-black text-gray-600 uppercase tracking-widest mt-1">Active Leads Assigned</p>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-xs font-black text-gray-600 uppercase tracking-widest mt-1">Total Sales Closed</p>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-xs font-black text-gray-600 uppercase tracking-widest mt-1">Follow-ups Due Today</p>
                    </div>
                </div>
            </div>
